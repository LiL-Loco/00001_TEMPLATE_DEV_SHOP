...
{foreach $Bestellung->Positionen as $Position}
    {if $Position->nPosTyp == 1}
        {if !empty($Position->kKonfigitem)} * {/if}{$Position->nAnzahl}x {$Position->cName} - {$Position->cGesamtpreisLocalized[$NettoPreise]}{if !empty($Position->Artikel->nErscheinendesProdukt)}
