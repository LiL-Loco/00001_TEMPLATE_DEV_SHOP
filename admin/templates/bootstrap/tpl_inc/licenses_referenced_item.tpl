...
<div id="license-item-{$license->getID()}-{$license->getLicense()->getType()}">
    ...
    {elseif $referencedItem !== null}
        ...
        {$disabled = $licData->isExpired()
        || ($subscription->isExpired() && !$subscription->canBeUsed())
        || (!$referencedItem->isFilesMissing() && !$referencedItem->canBeUpdated())}
        {$avail = $license->getReleases()->getAvailable()}
        ...
        {if $installedVersion === null || $referencedItem->isFilesMissing()}
            {if $avail === null}
                ...
                <span class="badge badge-danger">{__('No version available')}</span>
            {else}
                {if $avail->isPhpVersionTooLow()}
                    {$disabled = true}
                    <span class="badge badge-danger">{__('PHP version too low')}</span>
                    <br>
                {elseif $avail->isPhpVersionTooHigh()}
                    {$disabled = true}
            ...
            {form method="post" class="mt-2{if !$disabled} install-item-form{/if}"}
                <input type="hidden" name="noBootstrap" value="1">
            ...
            {/if}
                ...
                <div class="btn-group">
                    <button{if $disabled} disabled{/if}
                            class="btn btn-default btn-sm install-item"
                            name="action"
                            value="install">
                    ...
                    {foreach $license->getLinks() as $link}
                        {if $link->getRel() === 'itemDetails'}
                            <a class="btn btn-default btn-sm"
                               target="_blank"
                               rel="noopener"
                               href="{$link->getHref()}#tab-changelog">
        ...
        {else}
            ...
            {$installedVersion}
            {if $referencedItem->isActive() === false} {__('(disabled)')}{/if}
        {/if}
        {if $referencedItem->hasUpdate()}
            ...
            {if $referencedItem->canBeUpdated() === false}
                {if $avail !== null && $avail->isPhpVersionTooLow()}
                    <span class="badge badge-danger">{__('PHP version too low')}</span>
                {elseif $avail !== null && $avail->isPhpVersionTooHigh()}
                    <span class="badge badge-danger">{__('PHP version too high')}</span>
                {/if}
            ...
            {form method="post" class="mt-2{if !$disabled} update-item-form{/if}"}
                ...
                <div class="btn-group">
                    <button{if $disabled} disabled{/if}
                            class="btn btn-default btn-sm update-item"
                            name="action"
                            value="update">
