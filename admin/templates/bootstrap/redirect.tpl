...
cTitel=__('redirect')
cBeschreibung=__('redirectDesc')
cDokuURL=__('redirectURL')}
...
<div class="tabs">
    <nav class="tabs-nav">
        <ul class="nav nav-tabs" role="tablist">
            <li class="nav-item" role="presentation">
                <a class="nav-link {if $cTab === 'redirects'} active{/if}"
                   data-toggle="tab" role="tab" href="#redirects">
                    {__('overview')}
                </a>
            </li>
            <li class="nav-item" role="presentation">
                <a class="nav-link {if $cTab === 'new_redirect'} active{/if}"
                   data-toggle="tab" role="tab" href="#new_redirect">
                    {__('create/edit')}
                </a>
            </li>
        </ul>
    ...
    <div class="tab-content">
        <div role="tabpanel" class="tab-pane fade{if $cTab === 'redirects'} active show{/if}" id="redirects">
            ...
            {$types = [
            JTL\Redirect\Type::IMPORT => __('Import'),
            JTL\Redirect\Type::MANUAL => __('Manual'),
            JTL\Redirect\Type::WAWI => __('Wawi sync'),
            JTL\Redirect\Type::NOTFOUND => __('Not found'),
            JTL\Redirect\Type::UNKNOWN => __('Unknown')
            ]}
            <form method="post">
                {$jtl_token}
                    ...
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            <tr>
                                ...
                                <th class="min-w">
                                    {__('redirectTo')} {call sortControls pagination=$pagination nSortBy=1}
                                </th>
                                <th class="text-center">
                                    {__('calls')} {call sortControls pagination=$pagination nSortBy=2}
                                </th>
                                <th class="text-center">
                                    {__('Type')} {call sortControls pagination=$pagination nSortBy=3}
                                </th>
                                ...
                                <tr>
                                    <td>
                                        <div class="custom-control custom-checkbox">
                                            <input class="custom-control-input"
                                                   type="checkbox"
                                                   name="redirects[{$redirect->kRedirect}][enabled]"
                                                   value="1"
                                                   id="check-{$redirect->kRedirect}">
                                            <label class="custom-control-label" for="check-{$redirect->kRedirect}">
                                            </label>
                                        </div>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="form-group form-row align-items-center"
                                             id="input-group-{$redirect->kRedirect}">
                                            <span class="col col-lg-auto col-form-label text-info state-checking">
                                                <i class="fa fa-spinner fa-pulse"></i>
                                            </span>
                                            <span class="col col-lg-auto col-form-label text-success state-available"
                                                  style="display:none;">
                                                    <i class="fal fa-check"></i>
                                                </span>
                                            <span class="col col-lg-auto col-form-label text-danger state-unavailable"
                                                  style="display:none;">
                                                    <i class="fal fa-exclamation-triangle"></i>
                                                </span>
                                            <div class="col col-md-10">
                                                <input class="form-control min-w-sm"
                                                       name="redirects[{$redirect->kRedirect}][cToUrl]"
                                                       value="{$redirect->cToUrl}"
                                                       id="cToUrl-{$redirect->kRedirect}"
                                        ...
                                        <div class="btn-group">
                                            {if $redirect->nCount > 0}
                                                <button type="button" class="btn btn-link px-2"
                                                        ...
                                                        data-toggle="tooltip">
                                                    <span class="fal fa-chevron-circle-down rotate-180 font-size-lg">
                                                    </span>
                                                </button>
                                            {/if}
                                        ...
                                        <td colspan="5">
                                            <div id="referer-div-{$redirect->kRedirect}" style="display:none;">
                                                <table class="table">
                                                    ...
                                                    <tbody>
                                                    {foreach $redirect->referers as $redirectReferer}
                                                        <tr>
                                                            <td>
                                                                ...
                                                                {elseif $redirectReferer->cRefererUrl|strlen > 0}
                                                                    <a href="{$redirectReferer->cRefererUrl}"
                                                                       target="_blank">
        ...
        <div role="tabpanel" class="tab-pane fade{if $cTab === 'new_redirect'} active show{/if}" id="new_redirect">
            <form method="post">
                {$jtl_token}
                {$available = $currentRedirect->available|default:'n'}
                <div class="settings">
                    ...
                    <div>
                        <div class="form-group form-row align-items-center">
                            <label class="col col-sm-4 col-form-label text-sm-right"
                                   for="cFromUrl">{__('redirectFrom')}:</label>
                            <div class="col-sm pl-sm-3 pr-sm-5 order-last order-sm-2">
                                <input class="form-control" id="cFromUrl" name="cFromUrl" required
                                       {if $currentRedirect !== null}value="{$currentRedirect->source}"{/if}>
                            </div>
                        </div>
                        <div class="form-group form-row align-items-center" id="input-group-0">
                            <label class="col col-sm-4 col-form-label text-sm-right" for="cToUrl-0">
                                {__('redirectTo')}:
                            </label>
                            <div class="col-sm pl-sm-3 pr-sm-5 order-last order-sm-2">
                                <input class="form-control"
                                       ...
                                       required
                                       onblur="checkUrl(0)"
                                       {if $currentRedirect !== null}value="{$currentRedirect->destination}"{/if}>
                            </div>
                            <div class="col-auto ml-sm-n4 order-2 order-sm-3 state-checking"
                                    {if $available !== 'u'} style="display:none;"{/if}>
                                <i class="fa fa-spinner fa-pulse text-info"></i>
                            </div>
                            <div class="col-auto ml-sm-n4 order-2 order-sm-3 state-available"
                                    {if $available !== 'y'} style="display:none;{/if}">
                                <i class="fal fa-check text-success text-success"></i>
                            </div>
                            <div class="col-auto ml-sm-n4 order-2 order-sm-3 state-unavailable"
                                    {if $available !== 'n'} style="display:none;"{/if}>
                        ...
                        <div class="form-group form-row align-items-center">
                            ...
                            <div class="col-sm pl-sm-3 pr-sm-5 order-last order-sm-2">
                                <select class="custom-select" id="paramHandling" name="paramHandling">
                                    <option value="0"{if $paramHandling === 0} selected{/if}>
                                        {__('exact match')}
                                    </option>
                                    <option value="1"{if $paramHandling === 1} selected{/if}>
                                        {__('ignore GET parameters')}
                                    </option>
                                    <option value="2"{if $paramHandling === 2} selected{/if}>
                                        {__('append GET parameters')}
                                    </option>
                                </select>
                            </div>
                            <div class="col-auto ml-sm-n4 order-2 order-sm-3">
                                {getHelpDesc cDesc=__('parameterHandlingDesc')}
                            </div>
                        </div>
                    </div>
                    <div class="save-wrapper">
                            ...
                            {if !empty($redirectID)}
                                <div class="col-sm-6 col-xl-auto">
                                    <a class="btn btn-outline-primary btn-block"
                                       id="go-back" href="{$adminURL}{$route}">
                                        {__('cancelWithIcon')}
                                    </a>
                                </div>
                                <div class="col-sm-6 col-xl-auto">
