...
cTitel=__('livesearch')
cBeschreibung=__('livesucheDesc')
cDokuURL=__('livesucheURL')}
<div id="content">
    <div class="card">
        <div class="card-body">
    ...
    <nav class="tabs-nav">
        <ul class="nav nav-tabs" role="tablist">
            <li class="nav-item">
                <a class="nav-link {if !isset($tab) || $tab === 'suchanfrage'} active{/if}"
                   data-toggle="tab"
                   role="tab"
                   href="#suchanfrage">
                    {__('searchrequest')}
            ...
            <li class="nav-item">
                <a class="nav-link {if isset($tab) && $tab === 'erfolglos'} active{/if}"
                   data-toggle="tab"
                   role="tab"
                   href="#erfolglos">
                    {__('searchmiss')}
            ...
            <li class="nav-item">
                <a class="nav-link {if isset($tab) && $tab === 'mapping'} active{/if}"
                   data-toggle="tab"
                   role="tab"
                   href="#mapping">
                    {__('mapping')}
    ...
    <div class="tab-content">
                    ...
                    <div>
                                        ...
                                        <td>
                                            <div class="custom-control custom-checkbox">
                                                <input class="custom-control-input"
                                                       type="checkbox"
                                                       name="kSuchanfrage[]"
                                                       id="search-request-id-{$suchanfrage->kSuchanfrage}"
                                                       value="{$suchanfrage->kSuchanfrage}" />
                                                <label class="custom-control-label"
                                                       for="search-request-id-{$suchanfrage->kSuchanfrage}"></label>
                                            </div>
                                        </td>
                                        <td>{$suchanfrage->cSuche}</td>
                                        <td>
                                            <input class="form-control fieldOther"
                                                   name="nAnzahlGesuche_{$suchanfrage->kSuchanfrage}"
                                                   type="text" value="{$suchanfrage->nAnzahlGesuche}"
                                                   style="width:50px;" />
                                        </td>
                                        <td class="text-center">
                                            <div class="custom-control custom-checkbox">
                                                <input class="custom-control-input"
                                                       type="checkbox"
                                                       name="nAktiv[]"
                                                       id="nAktiv_{$suchanfrage->kSuchanfrage}"
                                                       value="{$suchanfrage->kSuchanfrage}" {if $suchanfrage->nAktiv==1}checked="checked"{/if} />
                                                <label class="custom-control-label"
                                                       for="nAktiv_{$suchanfrage->kSuchanfrage}"></label>
                                            </div>
                                        </td>
                                        <td class="text-center">
                        ...
                        <div class="save-wrapper">
                            <div class="row">
                                ...
                                <div class="col-sm-6 col-xl-auto">
                                    <button name="suchanfragenUpdate"
                                            type="submit"
                                            value="1"
        ...
        <div id="erfolglos" class="tab-pane fade {if isset($tab) && $tab === 'erfolglos'} active show{/if}">
            {if $Suchanfragenerfolglos && count($Suchanfragenerfolglos) > 0}
                ...
                <form name="login" method="post" action="{$adminURL}{$route}">
                    ...
                    <input type="hidden"
                           name="nErfolglosEditieren"
                           value="{if isset($nErfolglosEditieren)}{$nErfolglosEditieren}{/if}">
                    <div class="settings">
                                ...
                                {foreach $Suchanfragenerfolglos as $Suchanfrageerfolglos}
                                    <tr>
                                        <td>
                                            <div class="custom-control custom-checkbox">
                                                <input class="custom-control-input"
                                                       name="kSuchanfrageErfolglos[]"
                                                       type="checkbox"
                                                       id="search-request-unsuccessful-id-{$Suchanfrageerfolglos->kSuchanfrageErfolglos}"
                                                       value="{$Suchanfrageerfolglos->kSuchanfrageErfolglos}" />
                                                <label class="custom-control-label"
                                                       for="search-request-unsuccessful-id-{$Suchanfrageerfolglos->kSuchanfrageErfolglos}"></label>
                                            </div>
                                        </td>
                                        <td>
                                            {if isset($nErfolglosEditieren) && $nErfolglosEditieren == 1}
                                                <input class="form-control"
                                                       name="cSuche_{$Suchanfrageerfolglos->kSuchanfrageErfolglos}"
                                                       type="text"
                                                       value="{$Suchanfrageerfolglos->cSuche}" />
                                            {else}
                                        ...
                                        <td>
                                            {if !isset($nErfolglosEditieren) || $nErfolglosEditieren != 1}
                                                <input class="form-control fieldOther"
                                                       name="mapping_{$Suchanfrageerfolglos->kSuchanfrageErfolglos}"
                                                       type="text" />
                                            {/if}
                                        </td>
                                    </tr>
                                ...
                                <div class="col-sm-6 col-xl-auto text-left">
                                    <div class="custom-control custom-checkbox">
                                        <input class="custom-control-input"
                                               ...
                                               id="ALLMSGS2"
                                               type="checkbox"
                                               onclick="AllMessagesExcept(this.form, 'nAktiv_');" />
                                        <label class="custom-control-label" for="ALLMSGS2">
                                            {__('livesucheSelectAll')}
                                        </label>
                                    </div>
                                </div>
                                <div class="ml-auto col-sm-6 col-xl-auto">
                                ...
                                {foreach $Suchanfragenmapping as $sfm}
                                    <tr>
                                        <td>
                                            <div class="custom-control custom-checkbox">
                                                <input class="custom-control-input"
                                                       name="kSuchanfrageMapping[]"
                                                       type="checkbox"
                                                       id="search-mapping-id-{$sfm->kSuchanfrageMapping}"
                                                       value="{$sfm->kSuchanfrageMapping}">
                                                <label class="custom-control-label"
                                                       for="search-mapping-id-{$sfm->kSuchanfrageMapping}"></label>
                                            </div>
                                        </td>
                                        <td>{$sfm->cSuche}</td>
                                        <td>{$sfm->cSucheNeu}</td>
                                ...
                                <tr class="tab-1_bg">
                                    <td>
                                        <textarea class="form-control"
                                                  name="suchanfrageblacklist"
                                                  style="width:100%;min-height:400px;">{foreach $Suchanfragenblacklist as $Suchanfrageblacklist}{$Suchanfrageblacklist->cSuche};{/foreach}</textarea>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
        ...
        <div id="einstellungen" class="tab-pane fade {if isset($tab) && $tab === 'einstellungen'} active show{/if}">
            ...
            name='einstellen'
            a='saveSettings'
            action=$adminURL|cat:$route buttonCaption=__('saveWithIcon')
            title=__('settings') skipHeading=true tab='einstellungen'}
        </div>
    </div>
</div>
