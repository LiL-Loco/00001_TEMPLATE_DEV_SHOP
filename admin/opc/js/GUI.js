...
export class GUI
{
    ...
    selectFileProp(clickedBtn, propName, FileType)
    {
        let hiddenInput = window.configForm.querySelector('[name="' + propName + '"]');
        if (hiddenInput.value.length > 0) {
            hiddenInput.value = '';
            clickedBtn.dataset.selected = 'false';
        } else {
            this.openElFinder((file, mediafilesBaseUrlPath) => {
                hiddenInput.value = file.url.slice(mediafilesBaseUrlPath.length);
                clickedBtn.querySelector('.upload-btn-filename').innerText = file.name;
                clickedBtn.dataset.selected = 'true';
            }, FileType);
        }
    }
