...
class AccountController
{
    ...
    private function getDeliveryAddresses(
        ...
        foreach ($addresses->toArray() as $address) {
            /** @var DeliveryAddressTemplate $address */
            $allCountries[] = (string)$address->cLand;
        }
        $this->smarty->assign($smartyAssign[0], $addresses)
            ->assign(
                $smartyAssign[1],
                $this->shippingService->getPossibleShippingCountries(
                    \array_unique($allCountries),
                    $customer->getGroupID(),
    ...
    private function updateShippingAddress(Customer $customer): never
    {
        ...
        $template->nIstStandardLieferadresse = (int)($shipping_address['isDefault'] ?? 0);
