...
class Resources
{
    ...
    public function getMinifyArray(bool $absolute = false): array
    {
        ...
        foreach ($this->getGroups() as $name => $_tplGroup) {
            $res[$name]               = [];
            $this->versionList[$name] = [];
            foreach ($_tplGroup as $_file) {
                if (!\file_exists($_file['abs'])) {
                    continue;
                }
                $path = $absolute === true ? $_file['abs'] : $_file['rel'];

                $res[$name][] = $path;
                if (isset($_file['vsn'])) {
                    $this->versionList[$name][$path] = $_file['vsn'];
                }
            }
        }
