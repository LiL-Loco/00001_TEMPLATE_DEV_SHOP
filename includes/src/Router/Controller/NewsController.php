...
class NewsController extends AbstractController
{
    ...
    public function getStateFromSlug(array $args): State
    {
        ...
        if ($name !== null) {
            $parser = new DefaultParser($this->db, $this->state, $this->productFilter);
    ...
    public function getResponse(ServerRequestInterface $request, array $args, JTLSmarty $smarty): ResponseInterface
    {
        ...
        try {
            ...
            $response          = match ($this->getPageType($this->state->getAsParams())) {
                ViewType::NEWS_DETAIL         => $this->getNewsDetail(),
                ViewType::NEWS_CATEGORY       => $this->getNewsCategory(),
                ViewType::NEWS_OVERVIEW       => $this->getNewsOverview(),
                ViewType::NEWS_MONTH_OVERVIEW => $this->getNewsMonthOverview(),
                default                       => $this->notFoundResponse($request, $args, $smarty),
            };
        } catch (SpecialPageNotFoundException | NotFoundException) {
            return $this->notFoundResponse($request, $args, $smarty);
        }
     ...
     */
    private function getNewsDetail(): bool
    {
        ...
        if (Request::pInt('kommentar_einfuegen') > 0 && Form::validateToken()) {
    ...
    private function getNewsOverview(): bool
    {
    ...
    private function getNewsCategory(): bool
    {
        $pagination = new Pagination();
    ...
    private function getNewsMonthOverview(): bool
    {
        $pagination = new Pagination();
    ...
    protected function displayItem(Item $newsItem, Pagination $pagination): void
    {
        ...
        $comments       = $newsItem->getComments()->getThreadedItems()->filter(static function (Comment $item): bool {
    ...
    public function addComment(int $id, array $data): bool
    {
        ...
        if ($checkedOK) {
            ...
            $comment->cKommentar = Text::htmlentities(Text::filterXSS($text));
    ...
    public static function mapDateName(int|string $month, int|string $year, string $langCode): string
    {
        $month = (int)$month;
        $year  = (int)$year;
        $name  = '';
        ...
        if ($langCode === 'ger') {
            return match ($month) {
                1       => Shop::Lang()->get('january', 'news') . ',' . $year,
                2       => Shop::Lang()->get('february', 'news') . ' ' . $year,
                3       => Shop::Lang()->get('march', 'news') . ' ' . $year,
                4       => Shop::Lang()->get('april', 'news') . ' ' . $year,
                5       => Shop::Lang()->get('may', 'news') . ' ' . $year,
                6       => Shop::Lang()->get('june', 'news') . ' ' . $year,
                7       => Shop::Lang()->get('july', 'news') . ' ' . $year,
                8       => Shop::Lang()->get('august', 'news') . ' ' . $year,
                9       => Shop::Lang()->get('september', 'news') . ' ' . $year,
                10      => Shop::Lang()->get('october', 'news') . ' ' . $year,
                11      => Shop::Lang()->get('november', 'news') . ' ' . $year,
                12      => Shop::Lang()->get('december', 'news') . ' ' . $year,
                default => '',
            };
        }
        $name .= \date('F', \mktime(0, 0, 0, $month, 1, $year) ?: null) . ', ' . $year;
