...
class NewsletterImportController extends AbstractBackendController
{
    ...
    /**
     * @param string[] $data - csv header fields
     * @return string[]|int<-1> - valid fields, -1 if required field not present
     */
    private function checkformat(array $data): int|array
    {
        $allowed = ['cAnrede', 'cVorname', 'cNachname', 'cEmail'];
        $fmt     = [];
        $cnt     = \count($data);
        for ($i = 0; $i < $cnt; $i++) {
            if (!empty($data[$i]) && \in_array($data[$i], $allowed, true)) {
                $fmt[$i] = $data[$i];
            }
        }
    ...
    /**
     * @param string[] $fmt - csv header fields
    ...
    private function processImport(array $fmt, array $data): string
    {
        ...
        $recipient->kSprache     = Request::pInt('kSprache');
        $recipient->kKunde       = 0;
