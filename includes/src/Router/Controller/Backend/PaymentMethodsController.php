...
class PaymentMethodsController extends AbstractBackendController
{
    ...
    public function getResponse(ServerRequestInterface $request, array $args, JTLSmarty $smarty): ResponseInterface
    {
        ...
        // reset log
        if (
            ($action = Request::verifyGPDataString('a')) === 'logreset'
    ...
    private function stepLog(): void
    {
        ...
        $selectedLevel  = $this->addLevelSelect($filterStandard);
        $filterStandard->addDaterangefield(\__('Zeitraum'), 'dDatum', '', 'date');
        $searchfield = $filterStandard->addTextfield(
            \__('logSearch'),
            'cLog',
            Operation::CONTAINS,
            DataType::TEXT,
        ...
        if ($method !== null && \mb_strlen($method->cModulId) > 0) {
            $paginationPaymentLog = (new Pagination('standard'))
                ->setItemCount(ZahlungsLog::count($method->cModulId, $selectedLevel, $filterStandard->getWhereSQL()))
                ->assemble();
    ...
    private function getActiveCustomerGroups(Zahlungsart $paymentMethod): array
    {
        ...
        foreach (Text::parseSSKint($paymentMethod->cKundengruppen) as $customerGroupID) {
    ...
    {
        $levelSelect = $filter->addSelectfield(\__('logLevel'), 'nLevel', 0, 'level');
        $levelSelect->addSelectOption(\__('logAll'), 0);
        $levelSelect->addSelectOption(\__('logDebug'), \LOGLEVEL_DEBUG, Operation::EQUALS);
        $levelSelect->addSelectOption(\__('logNotice'), \LOGLEVEL_NOTICE, Operation::EQUALS);
        $levelSelect->addSelectOption(\__('logError'), \LOGLEVEL_ERROR, Operation::EQUALS);
