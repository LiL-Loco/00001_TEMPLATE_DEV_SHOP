...
class ExportQueueController extends AbstractBackendController
{
    ...
    private function holeExportformatCron(): array
    {
        ...
        foreach ($exports as $export) {
            ...
            if ($export->Waehrung !== null) {
                $export->Waehrung->kWaehrung = (int)$export->Waehrung->kWaehrung;
            }
            ...
            if ($export->Kundengruppe !== null) {
                $export->Kundengruppe->kKundengruppe = (int)$export->Kundengruppe->kKundengruppe;
                $export->Kundengruppe->nNettoPreise  = (int)$export->Kundengruppe->nNettoPreise;
            }
    ...
    private function getFrequency(int $hours): string|false
    {
        if ($hours <= 0) {
            return false;
        }
    ...
    private function stepCreateInsert(JTLSmarty $smarty, array &$messages): string
    {
        ...
        $freq                 = !empty($_POST['nAlleXStundenCustom'])
            ? Request::pInt('nAlleXStundenCustom')
            : Request::pInt('nAlleXStunden');
        $error                = new stdClass();
        $error->kExportformat = $id;
        $error->dStart        = Text::filterXSS(Request::pString('dStart'));
