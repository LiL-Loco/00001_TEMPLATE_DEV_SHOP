...
class DashboardController extends AbstractBackendController
{
    ...
    private function actionLogin(): ResponseInterface
    {
        ...
        $res = $this->account->login(Request::pString('benutzer'), Request::pString('passwort'));
        switch ($res) {
    ...
    public function handleUnauthenticated(string $uri): ResponseInterface
    {
        ...
        return $this->getSmarty()->assign('uri', $uri)
            ->assign('alertError', $this->alertService->alertTypeExists(Alert::TYPE_ERROR))
            ->assign('alertList', $this->alertService)
            ->getResponse('login.tpl');
    }

    private function handleTwoFA(string $uri): ResponseInterface
    {
        $_SESSION['AdminAccount']->TwoFA_active = true;
        // restore first generated token from POST
        $_SESSION['jtl_token'] = Request::pString('jtl_token');
        if (Request::postVar('TwoFA_code', '') !== '') {
            ...
            $this->getSmarty()->assign('alertError', true);
        } else {
    ...
    private function handleSafeMode(): void
    {
