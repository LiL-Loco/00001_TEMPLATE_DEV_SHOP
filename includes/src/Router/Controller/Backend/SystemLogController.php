...
class SystemLogController extends AbstractBackendController
{
    ...
    public function getResponse(ServerRequestInterface $request, array $args, JTLSmarty $smarty): ResponseInterface
    {
        ...
        return $smarty->assign('oFilter', $filter)
            ->assign('minLogLevel', Settings::intValue(Globals::SYSLOG_LEVEL))
    ...
    private function addLevelSelect(Filter $filter): int
    {
        ...
        $levelSelect->addSelectOption(\__('systemlogDebug'), Level::Debug->value, Operation::EQUALS);
        $levelSelect->addSelectOption(\__('systemlogNotice'), Level::Info->value, Operation::EQUALS);
        $levelSelect->addSelectOption(\__('systemlogWarning'), Level::Warning->value, Operation::EQUALS);
        $levelSelect->addSelectOption(\__('systemlogError'), Level::Error->value, Operation::GREATER_THAN_EQUAL);
    ...
    private function handleUpdates(): void
    {
        if (Request::verifyGPDataString('action') === 'clearsyslog') {
            ...
            $this->db->update(
                ...
                'cName',
                'systemlog_flag',
                (object)['cWert' => Request::pInt('minLogLevel')]
            );
    ...
    protected function preparePluginStateLog(): void
    {
        ...
        $this->getSmarty()->assign('pluginStateMapper', Shop::Container()->getPluginState())
    ...
    protected function prepareTemplateLog(): void
    {
        ...
        $order       = $pagination->getOrderSQL();
        $limit       = $pagination->getLimitSQL();
        $templateLog = $this->db->getObjects(
        ...
        $this->smarty->assign('templateLogPagination', $pagination)
