...
class CouponsController extends AbstractBackendController
{
     ...
     * @var string[]
     */
    private array $validCouponTypes = [
        Kupon::TYPE_STANDARD,
        Kupon::TYPE_SHIPPING,
    ...
    public function getResponse(ServerRequestInterface $request, array $args, JTLSmarty $smarty): ResponseInterface
    {
        ...
        $this->action = Request::verifyGPDataString('action');
        ...
        if (Form::validateToken()) {
            ...
            if (isset($_POST['action'])) {
                if (Request::pString('action') === 'speichern') {
                    $this->action = 'speichern';
                } elseif (Request::pString('action') === 'loeschen') {
                    $this->action = 'loeschen';
                }
        ...
        if ($this->action === 'bearbeiten') {
            ...
            if ($couponID > 0) {
                ...
                try {
                    ...
                    $this->alertService->addError(\__('errorInvalidCouponType'), 'errorInvalidCouponType');
                    $this->action = '';
                }
            }
        } elseif ($this->action === 'speichern' || Request::postVar('saveAndContinue')) {
            $coupon = $this->actionSave($languages);
        ...
        if (
            ...
            $this->displayOverview($tab, $this->action);
        }
    ...
    {
        if (!$this->isValidCouponType($type)) {
    ...
    private function getCouponCount(string $type, string $whereSQL = ''): int
    {
        ...
        foreach ($couponIDs as $couponID) {
            ...
            foreach ($languages as $language) {
                ...
                $localizedName = Request::postVar($postVarName, '') !== ''
                    ? \htmlspecialchars(Request::pString($postVarName), \ENT_COMPAT | \ENT_HTML401, \JTL_CHARSET)
                    : $name;
    ...
    public function doImport(): void
    {
        ...
        $import->import('kupon', function (stdClass $obj, &$importDeleteDone, $importType = 2): bool {
            ...
            if (
                isset($obj->cKuponTyp)
                && $obj->cKuponTyp !== Kupon::TYPE_NEWCUSTOMER
    ...
    {
        $type = Request::verifyGPDataString('cKuponTyp');
        if (Request::hasGPCData('tab')) {
            $tab = Request::verifyGPDataString('tab');
        } elseif ($type !== '' && $this->isValidCouponType($type)) {
            $tab = $type;
        }
        ...
        if ($action === 'csvExport' && $this->isValidCouponType($exportID) && Form::validateToken()) {
            $export = new Export();
        ...
        if ($coupon->kKupon > 0) {
            ...
            foreach ($languages as $language) {
                ...
                $names[$language->getIso()] = Request::postVar($postVarName, '') !== ''
                    ? Text::filterXSS(Request::pString($postVarName))
                    : $coupon->cName;
            }
        }
     ...
     * @param LanguageModel[] $languages
     */
    private function actionSave(array $languages): Kupon
    {
        $coupon       = $this->createCouponFromInput();
        $couponErrors = $coupon->validate();
        if (!$this->isValidCouponType($coupon->cKuponTyp)) {
            $coupon->cKuponTyp = Kupon::TYPE_STANDARD;
            $couponErrors[]    = \__('errorInvalidCouponType');
        }
    ...
    private function isValidCouponType(string $type): bool
    {
        return \in_array($type, $this->validCouponTypes, true);
    }
}
