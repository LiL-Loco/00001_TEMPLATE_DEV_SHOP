...
class BoxController extends AbstractBackendController
{
    ...
    public function getResponse(ServerRequestInterface $request, array $args, JTLSmarty $smarty): ResponseInterface
    {
        ...
        if (Request::pInt('einstellungen') > 0) {
            ...
            match ($_REQUEST['action']) {
                'delete-invisible' => $this->actionDeleteInvisible(Request::pIntArray('kInvisibleBox')),
                'new'              => $this->actionNew(
                    $boxID,
                    $pageID,
                    Request::pInt('container'),
                    Text::filterXSS(Request::pString('position'))
                ),
                'del'              => $this->actionDelete($boxID),

                'edit_mode'        => $this->actionEditMode($boxID),
                'edit'             => $this->actionEdit($boxID, $linkID),
                'resort'           => $this->actionResort($pageID),
                'activate'         => $this->actionActivate($boxID),
    ...
    private function actionResort(int $pageID): void
    {
        ...
        foreach ($boxes as $i => $boxIDtoSort) {
            ...
            $this->filterBoxVisibility($boxIDtoSort, $pageID, Request::pString($idx));
        }
    ...
    /**
     * @param numeric-string[]|string $filter
     */
    public function filterBoxVisibility(int $boxID, int $pageID, array|string $filter = ''): int
    {
        // make sure this is a comma separated list of integers
        $filter = \is_array($filter)
            ? \implode(',', \array_map('\intval', \array_unique($filter)))
            : \implode(',', \array_unique(\array_filter(\array_map('\intval', \explode(',', $filter)))));
