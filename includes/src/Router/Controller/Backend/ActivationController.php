...
class ActivationController extends AbstractBackendController
{
    ...
    private function getAction(): void
    {
        ...
        if (Request::verifyGPCDataInt('bewertungen') === 1) {
            $this->handleReviews();
        } elseif (Request::verifyGPCDataInt('suchanfragen') === 1) {
            $this->handleSearchQueries();
        } elseif (Request::verifyGPCDataInt('newskommentare') === 1) {
            $this->handleNewsComments();
        } elseif (Request::verifyGPCDataInt('newsletterempfaenger') === 1) {
            $this->handleNewsletterRecipients();
        }
    }

    private function handleReviews(): void
    {
        if (Request::pInt('freischaltensubmit') === 1) {
            if ($this->activateReviews(Request::pIntArray('kBewertung'))) {
        ...
        } elseif (Request::pInt('freischaltenleoschen') === 1) {
            if ($this->deleteReviews(Request::pIntArray('kBewertung'))) {
    ...
    private function handleSearchQueries(): void
    {
        if (Request::pInt('submitMapping') === 1) {
            ...
            $queryIDs = Request::pIntArray('kSuchanfrage');
            if (\count($queryIDs) === 0) {
            ...
            $res = $this->mapLiveSearch($queryIDs, $mapping);
            if ($res !== 1) {
            ...
            if (!$this->activateSearchQueries(Request::pIntArray('kSuchanfrage'))) {
                $this->alertService->addError(
        ...
        if (Request::pInt('freischaltenleoschen') === 1) {
            if ($this->deleteSearchQueries(Request::pIntArray('kSuchanfrage'))) {
    ...
    private function handleNewsComments(): void
    {
        if (Request::pInt('freischaltensubmit') === 1) {
            /** @var numeric-string[] $arr */
        ...
        if (Request::pInt('freischaltenleoschen') === 1) {
            if ($this->deleteNewsComments(Request::pIntArray('kNewsKommentar'))) {
    ...
    private function handleNewsletterRecipients(): void
    {
        if (Request::pInt('freischaltensubmit') === 1) {
            if ($this->activateNewsletterRecipients(Request::pIntArray('kNewsletterEmpfaenger'))) {
        ...
        if (Request::pInt('freischaltenleoschen') === 1) {
            if ($this->deleteNewsletterRecipients(Request::pIntArray('kNewsletterEmpfaenger'))) {
    ...
    private function mapLiveSearch(array $queryIDs, string $mapTo): int
    {
        ...
        foreach (\array_map('\intval', $queryIDs) as $queryID) {
            ...
            if ($db->insert('tsuchanfragemapping', $mapping) < 1) {
