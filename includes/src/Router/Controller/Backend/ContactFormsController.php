...
class ContactFormsController extends AbstractBackendController
{
    ...
    public function getResponse(ServerRequestInterface $request, array $args, JTLSmarty $smarty): ResponseInterface
    {
        ...
        if (Form::validateToken()) {
            if (Request::gInt('del') > 0) {
                $this->actionDeleteItem(Request::gInt('del'));
            }
            if (Request::pInt('content') === 1) {
                $this->actionCreateItem();
                $tab = 'content';
            }
            if (Request::pInt('betreff') === 1) {
            ...
            if ((Request::gInt('kKontaktBetreff') > 0 || Request::getInt('neu') === 1)) {
                $this->step = 'betreff';
            }
        }
    ...
    private function actionCreateItem(): void
    {
        ...
        foreach (LanguageHelper::getAllLanguages(0, true, true) as $language) {
            ...
            $spezialContent1->cContent        = Request::pString('cContentTop_' . $code);
            $spezialContent2->cContent        = Request::pString('cContentBottom_' . $code);
            $spezialContent3->cContent        = \htmlspecialchars(
                Request::pString('cTitle_' . $code),
