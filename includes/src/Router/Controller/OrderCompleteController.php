...
class OrderCompleteController extends CheckoutController
{
    ...
    public function getResponse(ServerRequestInterface $request, array $args, JTLSmarty $smarty): ResponseInterface
    {
        ...
        $order      = null;
        if (isset($_GET['i'])) {
            $orderID = $this->db->select('tbestellid', 'cId', Request::gString('i'));
            if ($orderID !== null && $orderID->kBestellung > 0) {
                $orderID->kBestellung = (int)$orderID->kBestellung;
                $order                = new Bestellung($orderID->kBestellung, false, $this->db);
                $order->fuelleBestellung(false);
                $handler->saveUploads($order);
                $this->db->delete('tbestellid', 'kBestellung', $orderID->kBestellung);
            }
        ...
        $pluginID = Helper::getIDByModuleID($order->Zahlungsart->cModulId ?? '');
        if ($pluginID > 0) {
    ...
    protected function handlePayAgain(int $orderID): ResponseInterface
    {
        ...
        $bestellid = $this->db->select('tbestellid', 'kBestellung', (int)$order->kBestellung);
        ...
        if (Request::verifyGPCDataInt('zusatzschritt') === 1) {
            ...
            if ($moduleID === 'za_lastschrift_jtl') {
                if (
                    ...
                    $_SESSION['Zahlungsart']->ZahlungsInfo->cBankName =
                        Text::htmlentities(\stripslashes(Request::pString('bankname')), \ENT_QUOTES);
                    $_SESSION['Zahlungsart']->ZahlungsInfo->cKontoNr  =
                        Text::htmlentities(\stripslashes(Request::pString('kontonr')), \ENT_QUOTES);
                    $_SESSION['Zahlungsart']->ZahlungsInfo->cBLZ      =
                        Text::htmlentities(\stripslashes(Request::pString('blz')), \ENT_QUOTES);
                    $_SESSION['Zahlungsart']->ZahlungsInfo->cIBAN     =
                        Text::htmlentities(\stripslashes(Request::pString('iban')), \ENT_QUOTES);
                    $_SESSION['Zahlungsart']->ZahlungsInfo->cBIC      =
                        Text::htmlentities(\stripslashes(Request::pString('bic')), \ENT_QUOTES);
                    $_SESSION['Zahlungsart']->ZahlungsInfo->cInhaber  =
                        Text::htmlentities(\stripslashes(Request::pString('inhaber')), \ENT_QUOTES);
            ...
            if ($hasAdditionalInformation) {
                ...
                if ($handler->savePaymentInfo((int)$order->kKunde, (int)$order->kBestellung)) {
                    $this->db->update(
                        'tbestellung',
                        'kBestellung',
            ...
            try {
                ...
                if ($paymentMethod !== null) {
                    if ($paymentMethod->validateAdditional()) {
                        ...
                        $order->Zahlungsart = $this->getPaymentMethod($order->kZahlungsart ?? 0);
                    }
                }
    ...
    public function isOrderComplete(?Cart $cart = null): bool
    {
        $validationData          = new CheckboxValidationDomainObject(
            Typifier::intify($this->customerGroupID),
    ...
    public function setzeSmartyWeiterleitung(Bestellung $order): void
    {
        ...
        if ($pluginID > 0) {
            ...
            try {
                ...
                $logger->error('Associated plugin for payment method {mid} not found', ['mid' => $moduleID]);
                return;
            }
            global $oPlugin;
            $oPlugin             = $plugin;
