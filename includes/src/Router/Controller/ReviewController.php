...
class ReviewController extends PageController
{
    ...
    public function handleRequest()
    {
        ...
        if (Request::verifyGPCDataInt('bfa') === 1) {
            return $this->reviewPreCheck($customer, $this->state->productID, $this->state->sortID);
        }
    }
    ...
    private function save(int $productID, int $customerID, int $langID, string $title, string $text, int $stars): string
    {
        ...
        $review->name       = $customer->cVorname . ' ' . \mb_substr((string)$customer->cNachname, 0, 1);
        $review->title      = $title;
        ...
        if ($this->config['bewertung']['bewertung_freischalten'] === 'N') {
            ...
            return $url . (($reward > 0)
                    ? '&fB=' . $reward . '&cHinweis=h04'
                    : '&cHinweis=h01');
        }

        return $url . '&cHinweis=h05';
    }

    private function reviewPreCheck(Customer $customer, int $productID, int $sort): bool
    {
        $reviewAllowed = true;
        ...
        $this->currentProduct->fuelleArtikel($productID, Artikel::getDefaultOptions());
