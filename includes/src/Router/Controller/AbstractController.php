...
abstract class AbstractController implements ControllerInterface
{
    ...
    public function init(): bool
    {
        ...
        $deliveryCountry = $deliveryAddress->cLand ?? '';
        $this->getShippingService()->setTaxRateByDeliveryCountry($deliveryCountry);
    ...
    public function getStateFromSlug(array $args): State
    {
        ...
        if ($name !== null) {
            ...
            $name      = $parser->parse((string)$name, $args, $routeType);
        }
        $seo = $id > 0
    ...
    public function preRender(): void
    {
        ...
        $deliveryAddress    = $this->getShippingService()->getDeliveryAddress($customer);
        $deliveryCountry    = $deliveryAddress->cLand ?? '';
        $deliveryZipCode    = $deliveryAddress->cPLZ ?? '';
        $favourableShipping = $this->getShippingService()->getFavourableShippingMethod(
        ...
        $freeShippingMethod = (
            $favourableShipping !== null
            && $this->getShippingService()->isZero($favourableShipping->finalNetCost)
        )
            ? null
            : $this->getShippingService()->getFreeShippingMethod(
                $customer,
                $customerGroup,
                $currency,
                $deliveryCountry,
        ...
        $this->smarty->assign('linkgroups', $linkHelper->getVisibleLinkGroups())
            ...
            ->assign(
                ...
                $freeShippingMethod !== null
                    ? $this->getShippingService()->getShippingFreeString(
                        $freeShippingMethod,
            ...
            ->assign('shippingCountry', $deliveryCountry)
            ->assign('VersandPLZ', $deliveryZipCode)
            ->assign(
