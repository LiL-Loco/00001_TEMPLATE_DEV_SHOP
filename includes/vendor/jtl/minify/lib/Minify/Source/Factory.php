...
class Minify_Source_Factory
{
     ...
     *
     * @param Minify_CacheInterface|null $cache Optional cache for handling .less files.
     */
    public function __construct(Minify_Env $env, array $options = [], ?Minify_CacheInterface $cache = null)
    {
        $this->env     = $env;
        $this->options = array_merge([
            'noMinPattern'        => '@[-\\.]min\\.(?:[a-zA-Z]+)$@i', // matched against basename
            'fileChecker'         => [$this, 'checkIsFile'],
            'resolveDocRoot'      => true,
            'checkAllowDirs'      => true,
            'allowDirs'           => ['//'],
    ...
    public function makeSource($spec)
    {
        ...
        if ($this->options['resolveDocRoot'] && str_starts_with($spec['filepath'], '//')) {
        ...
        if ($this->options['checkAllowDirs']) {
            ...
            foreach ($allowDirs as $allowDir) {
                if (str_starts_with($filePath, $this->env->normalizePath($allowDir))) {
                    $inAllowedDir = true;
                }
            }
