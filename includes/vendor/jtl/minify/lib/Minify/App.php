...
class App extends Container
{
    public function __construct(string $dir)
    {
        ...
        $this->cache = static function (App $app) use ($that) {
            $config = $app->config;
        ...
        $this->config = static function (App $app) {
            ...
            $varNames = \array_filter($varNames, static function ($name) use ($varDefined) {
        ...
        $this->controller = static function (App $app) use ($that) {
            $config = $app->config;
        ...
        $this->docRoot = static function (App $app) {
            $config = $app->config;
        ...
        $this->env = static function (App $app) {
            return new Minify_Env($app->config->envArgs);
        };

        $this->errorLogHandler = static function () {
        ...
        $this->groupsConfig = static function (App $app) {
            return (require $app->groupsConfigPath);
        };

        $this->groupsConfigPath = $this->dir . '/groupsConfig.php';

        $this->logger = static function (App $app) use ($that) {
        ...
        $this->minify = static function (App $app) use ($that) {
            $config = $app->config;
        ...
        $this->serveOptions = static function (App $app) {
            $config = $app->config;
            $env    = $app->env;
            ...
            foreach ($config->symlinks as $target) {
        ...
        $this->sourceFactory = static function (App $app) {
            return new Minify_Source_Factory($app->env, $app->sourceFactoryOptions, $app->cache);
        };

        $this->sourceFactoryOptions = static function (App $app) {
    ...
    public function runServer(): void
    {
        if (!$this->env->get('f') && $this->env->get('g') === null) {
            ...
            $url      = 'https://github.com/mrclay/minify/blob/master/docs/CommonProblems.wiki.md#'
