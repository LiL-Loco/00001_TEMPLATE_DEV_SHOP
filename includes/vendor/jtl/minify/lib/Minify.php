...
class Minify
{
    ...
    public const TYPE_JS   = 'application/x-javascript';
    public const URL_DEBUG = 'https://github.com/mrclay/minify/blob/master/docs/Debugging.wiki.md';
     ...
     */
    private Minify_CacheInterface $cache;
    ...
    public function getDefaultOptions()
    {
        return [
            ...
            'minifiers'       => [
                self::TYPE_JS   => ['JSMin\\JSMin', 'minify'],
                self::TYPE_CSS  => ['Minify_CSSmin', 'minify'],
                self::TYPE_HTML => ['Minify_HTML', 'minify'],
            ],
             ...
             */
            'importWarning'        => '/* See https://github.com/mrclay/minify/blob/master/docs/CommonProblems'
                . ".wiki.md#imports-can-appear-in-invalid-locations-in-combined-css-files */\n"
        ];
    }
     ...
     *
     */
    public function serve(Minify_ControllerInterface $controller, $options = [])
    {
        ...
        if (!$this->sources) {
            ...
            if (!$this->options['quiet']) {
                ...
                [, $statusCode] = explode(' ', $this->options['badRequestHeader']);
        ...
        if ($this->options['encodeOutput']) {
            ...
            if ($this->options['encodeMethod'] !== null) {
                ...
                [$this->options['encodeMethod'], $contentEncoding] = $list;
        ...
        if ($this->options['concatOnly']) {
            ...
            foreach ($this->sources as $source) {
                if ($this->options['contentType'] === self::TYPE_JS) {
                    ...
                    $source->setMinifier(['Minify_CSSmin', 'minify']);
     ...
     * @return string
     */
    public function combine($sources, $options = [])
    {
        $tmpCache    = $this->cache;
     ...
     * @access private
     */
    public function errorExit($header, $url = '', $msgHtml = ''): never
    {
    ...
    {
        foreach ($this->sources as $source) {
            $source->setMinifier(['Minify_Lines', 'minify']);
            $id = $source->getId();
    ...
    protected function combineMinify()
    {
        ...
        $defaultMinifier = $this->options['minifiers'][$type] ?? false;
        ...
        do {
            ...
            if (
                ...
                if ($lastMinifier) {
                    try {
                        $content[] = $lastMinifier($imploded, $lastOptions);
                    } catch (Exception $e) {
        ...
        if ($type === self::TYPE_CSS && str_contains($content, '@import')) {
     ...
     * @return array options for Minify
     */
    protected function analyzeSources($options = [])
    {
        if (!$this->sources) {
            return $options;
        }
            ...
            if (!empty($options['contentType'])) {
                ...
                if ($sourceType !== null && $sourceType !== $options['contentType']) {
                    $this->logger && $this->logger->warning(
                        "ContentType mismatch: '{$sourceType}' != '{$options['contentType']}'"
                    );
