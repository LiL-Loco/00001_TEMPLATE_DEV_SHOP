...
class QRCode
{
    ...
    protected int $errorCorrectLevel;

    protected int|float $lostPoint;

    protected int $darkCount;

    protected int|float $minLostPoint;

    protected int $maskPattern;

    protected BitBuffer $bitBuffer;

    protected QRDataInterface $qrDataInterface;

    protected QROutputInterface $qrOutputInterface;

    public function __construct(string $data, QROutputInterface $output, ?QROptions $options = null)
    {
     ...
     */
    public function setData(string $data, ?QROptions $options = null): self
    {
        $data = \trim($data);
    ...
    protected function testPattern(int $pattern): void
    {
    ...
    protected function setTypeNumber(bool $test): void
    {
    ...
    protected function setTypeInfo(bool $test, int $pattern): void
    {
        $this->setPattern();
    ...
    protected function createData(): void
    {
        ...
        if ($this->bitBuffer->length > $MAX_BITS) {
            throw new QRCodeException(
                'code length overflow. (' . $this->bitBuffer->length . ' > ' . $MAX_BITS . 'bit)'
            );
        }
    ...
    protected function createBytes(): array
    {
        ...
        foreach ($rsBlocks as $key => $value) {
            [$rsBlockTotal, $rsBlockDataCount] = $value;
     ...
     * @throws QRCodeException
     */
    protected function mapData(int $pattern): void
    {
        $this->createData();
    ...
    protected function setupPositionAdjustPattern(): void
    {
        ...
        foreach ($range as $posI) {
            foreach ($range as $posJ) {
     ...
     * @throws QRCodeException
     */
    protected function getMatrix(bool $test, int $maskPattern): void
    {
