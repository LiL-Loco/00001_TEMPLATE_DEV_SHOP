                            ...
                            <ul class="pagination">
                                ...
                                {block name='snippets-productlist-page-nav-last-page'}
                                    <li class="page-item{if $Suchergebnisse->getPages()->getCurrentPage() == $Suchergebnisse->getPages()->getMaxPage()} disabled{/if}">
                                        {link class="page-link js-pagination-ajax" href=$filterPagination->getNext()->getURL()|default:"#" aria=['label' => {lang key='next' section='productOverview'}, 'diabled' => "{if $Suchergebnisse->getPages()->getCurrentPage() == $Suchergebnisse->getPages()->getMaxPage()}true{else}false{/if}"]}<i class="fas fa-long-arrow-alt-right"></i>{/link}
                                    </li>
                                {/block}
                            </ul>
                        </nav>
