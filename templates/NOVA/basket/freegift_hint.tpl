        ...
        {if $freeGiftService->basketHoldsFreeGift(JTL\Session\Frontend::getCart()) === false}
            ...
            <div class="font-bold">
                {if !empty($oSpezialseiten_arr) && isset($oSpezialseiten_arr[$smarty.const.LINKTYP_GRATISGESCHENK])}
                    <a href="{$oSpezialseiten_arr[$smarty.const.LINKTYP_GRATISGESCHENK]->getURL()}"
                       title="{lang key='freeGiftsSeeAll' section='basket'}"><i class="fas fa-gifts text-gray-900 mr-1"></i></a>
                {else}
                    <i class="fas fa-gifts text-gray-900 mr-1"></i>
                {/if}
                <span>{lang key='freeGiftsAvailable' section='basket'}</span>
            </div>

            <span class="block">{lang section='basket' key='freeGiftsAvailableText'}</span>
            <a href="{get_static_route id='warenkorb.php'}#freeGiftsHeading" class="text-blue-600 underline p-0">
            ...
            {block name='basket-freegift-hint-still-missing-amount'}
                ...
                && !empty($nextFreeGiftMissingAmount)}
                    <span class="block">{lang section='basket' key='freeGiftsStillMissingAmountForNextFreeGift'
